"""
PTQ4ViT - DeiT-Small W8A8 Quantization
실행 위치: PTQ4ViT/ 루트에서
...
from utils.net_wrap import wrap_modules_in_net
...
CIFAR_DIR = "/scratch/x3433a06/cifar10"   # 자동 다운로드됨
N_CALIB   = 32
DEVICE    = "cuda"
# ─────────────────────────────────────────────────────
...
def get_cifar_calib_loader():
    """CIFAR-10을 224x224로 resize해서 calibration 데이터로 사용"""
...
def main():
    ...
    # 2. Quantization layer 교체 (PTQ4ViT 공식 방식)
    print("[2/4] Wrapping quantization layers...")
    wrapped_modules = wrap_modules_in_net(model, ptq_cfg)
    print(f"      Wrapped {len(wrapped_modules)} modules.")

    # 3. Calibration (CIFAR-10 32장)
    ...
    calibrator = HessianQuantCalibrator(
        ...
        sequential=False,   # parallel calibration (PTQ4ViT 기본값)
        batch_size=1,
    )
